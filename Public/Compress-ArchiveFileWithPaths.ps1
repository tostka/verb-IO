# Compress-ArchiveFileWithPaths.ps1

#*------v Function Compress-ArchiveFileWithPaths v------
function Compress-ArchiveFileWithPaths {
    <#
    .SYNOPSIS
    Compress-ArchiveFileWithPaths - WindowsPowershell code wrapper for [System.IO.Compression.ZipFile] that a compressed archive/zipped file _with_ stored releative paths (which compress-ARchive native cmd cannot do).
    .NOTES
    Author: Taylor Gibb
    ...
    AddedCredit : user2349693
    AddedWebsite: https://stackoverflow.com/questions/51392050/compress-archive-and-preserve-relative-paths
    AddedTwitter: URL
    REVISIONS   :
    * 1:02 PM 1/16/2024 COMPLATELY UNFINISHED - abandoned and diverted to explorer native compression, it does relative path compress wo all the ho9rse hockey, save finishintg this for later ; convert the below into an adv function
    * 11/24/19 user2349693's posted scriptblock from stackoverflow.com
    .DESCRIPTION
    Compress-ArchiveFileWithPaths - WindowsPowershell code wrapper for [System.IO.Compression.ZipFile] that a compressed archive/zipped file _with_ stored releative paths (which compress-ARchive native cmd cannot do).
    ...
    .EXAMPLE
    PS> $zipped = Compress-ArchiveFileWithPaths -path 'c:\tmp\20170411-0706AM.ps1','c:\tmp\24SbP1B9.ics' -DestinationPath "c:\tmp\test$((get-date -format 'yyyyMMdd-HHmmtt')).zip" -verbose  ;
    ...
    .EXAMPLE
    PS> $zipped = 'c:\tmp\20170411-0706AM.ps1','c:\tmp\24SbP1B9.ics' | Compress-ArchiveFileWithPaths -DestinationPath "c:\tmp\test$((get-date -format 'yyyyMMdd-HHmmtt')).zip"  -verbose ;
    Pipeline example: Expand content of specified file to DestinationPath
    .EXAMPLE
    PS> $zipped = Compress-ArchiveFileWithPaths -path C:\tmp\tmp\* -DestinationPath "c:\tmp\test$((get-date -format 'yyyyMMdd-HHmmtt')).zip" -verbose  ;
    ...
    PS>      Compress-ArchiveFileWithPaths -DestinationPath "d:\scripts\$($ticket)-work-files-$(get-date -format 'yyyyMMdd-HHmmtt').zip" ; 
...
#*------^ END Function Compress-ArchiveFileWithPaths ^------
