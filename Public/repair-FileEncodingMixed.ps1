...
function repair-FileEncodingMixed {
    ...
    AddedTwitter:	URL
    REVISIONS
    * 10:23 AM 9/3/2025 made the BackupFileNameSuffix an overridable param;
        ...
        foreach($Pth in $Path) {                
            ...
            TRY {
                ...
                if($Pth | select-string '\0'){
                    ...
                    $smsg = "SKIPPPING:`n$($sfile.fullname) has *NO* mixed-encoding (does *not* contain '\0' chars)" ; 
                    $smsg += "`n(returning original unmodified file path to piipeline)" ;
